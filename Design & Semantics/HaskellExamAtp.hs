...
scatterGather oob indices values 
        |null indices = []
        |(head indices) < (length values) = values !! head indices : scatterGather oob (drop 1 indices) values
        |otherwise = oob : scatterGather oob (drop 1 indices) values 
...
tear :: (b -> Bool) -> [b] -> [[b]]
tear p list = (filter p list) : (filter (not.p) list):[]
...
mapSkip func list
        |null list = []
        |(length list) > 1 = func(head list) : list !! 1 : mapSkip func (drop 2 list)
        |otherwise = func (head list) : []
